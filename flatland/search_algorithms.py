...
def breadth_first_search(start, goal, map_world):
    maps_rows = len(map_world)
    map_cols = len(map_world[0]) 
    ...
    while q:
        ...
        for next in adj.get(curr, []):
            if next not in visited: ##check if its cehcked this one yet to prevent cehcking it tweice
                visited.add(next)
                path_back[next] = curr
                q.append(next)
...
def main():
    ...
    goal = (3,3)
    ####
    breadth_first_search(start, goal, map_array)
