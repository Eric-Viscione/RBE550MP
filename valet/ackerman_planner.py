...
def generate_successors_diwheel(current):
    ...
    for dx, dy in move_directions:
        new_x = current.x + dx * step_size
        new_y = current.y + dy * step_size
...
def generate_successors_with_trailer(current,trailer_size=None):
    successors = []
    ...
    for steer in np.linspace(-MAX_STEER, MAX_STEER, num=5):
        ...
        for direction in [1, -1]:
            if abs(steer) < 1e-4:  # Straight motion
                ...
                radius = TURNING_RADIUS / math.cos(steer)
                dtheta = (STEP_SIZE / radius) * direction
                dx = radius * (math.sin(current.theta + dtheta) - math.sin(current.theta))
                dy = -radius * (math.cos(current.theta + dtheta) - math.cos(current.theta))
            ...
            articulation_angle = steer * trailer_size[0] / (TURNING_RADIUS + STEP_SIZE)

            trailer_x = new_x - trailer_size[0] * math.cos(new_theta + articulation_angle)
            trailer_y = new_y - trailer_size[0] * math.sin(new_theta + articulation_angle)

            cost = current.cost + STEP_SIZE  # constant cost per step
            
            new_node = Node(new_x, new_y, new_theta, cost, current, direction, vehicle_type=current.vehicle_type)
            new_node.trailer_x = trailer_x
            new_node.trailer_y = trailer_y
...
def hybrid_a_star(start, goal, grid, trailer=False, trailer_size=None):
    ...
    while open_set:
        ...
        if vehicle_type == "trailer":
            for neighbor in generate_successors_with_trailer(current, trailer_size):
                if is_collision(neighbor.x, neighbor.y, grid):
                    continue
                if path_collision_check(current.x, current.y, neighbor.x, neighbor.y, grid):
                    continue
                if is_collision(neighbor.trailer_x, neighbor.trailer_y, grid):
                    continue
                trailer_radius = (trailer_size[1] / 2.0) if len(trailer_size) > 1 else (XY_RESOLUTION)
                if is_collision_with_radius(neighbor.trailer_x, neighbor.trailer_y, grid, trailer_radius):
                    continue
                if path_collision_check(current.trailer_x, current.trailer_y, neighbor.trailer_x, neighbor.trailer_y, grid):
                    continue

                
                count += 1
                f_neighbor = neighbor.cost + heuristic(neighbor, goal, trailer=True)
...
def trailer_center(coord, vehicle_size, trailer_size):
    x, y, theta = coord  # now theta is used
    vehicle_length, vehicle_width = vehicle_size
    trailer_length, trailer_width = trailer_size

    offset = (vehicle_length / 2) + (trailer_length / 2)
    trailer_x = x - offset * math.cos(theta)
    trailer_y = y - offset * math.sin(theta)
    return trailer_x, trailer_y

def trailer_planner(start,goal, grid, vehicle_size, trailer_size):
    start_time = time.time()
    trailer_x, trailer_y = trailer_center(start, vehicle_size, trailer_size)
    start_node = Node(x=start[0], y=start[1], theta=np.deg2rad(start[2]),
                      trailer_x=trailer_x, trailer_y=trailer_y ,cost=0.0, 
                      parent=None, direction=1, vehicle_type="trailer")
    trailer_goal_x, trailer_goal_y = trailer_center(goal, vehicle_size, trailer_size)
    goal_node = Node(x=goal[0], y=goal[1], theta=np.deg2rad(goal[2]),
                    trailer_x=trailer_goal_x, trailer_y=trailer_goal_y,
                    cost=0.0, parent=None, direction=1, vehicle_type="trailer")
    inflated_grid = cspace(grid, vehicle_size)
    path = hybrid_a_star(start_node, goal_node, inflated_grid, trailer=True, trailer_size=trailer_size)
    end_time = time.time()
    elapsed_time = end_time - start_time
    print(f"Planned path in {elapsed_time} seconds")
    if path:
        path = reconstruct_path(path)
        animate_path(path, grid, vehicle_size, trailer_size, vehicle_type="Trailer Vehicle")
    else:
        visualize_path(None, grid, start, goal, vehicle_size, trailer_size)
